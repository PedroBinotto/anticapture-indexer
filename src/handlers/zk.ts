...
indexer.onEvent({ contract: "ZKToken", event: "Transfer" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "ZKToken", event: "DelegateChanged" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "ZKToken", event: "DelegateVotesChanged" }, async ({ event, context }) => {
...
// Governor handlers
indexer.onEvent({ contract: "ZKGovernor", event: "VoteCast" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "ZKGovernor", event: "ProposalCreated" }, async ({ event, context }) => {
  await proposalCreated(context, daoId, blockTime, {
    ...
    values: [...event.params.values], signatures: [],
...
indexer.onEvent({ contract: "ZKGovernor", event: "ProposalCanceled" }, async ({ event, context }) => {
  await updateProposalStatus(context, event.params.proposalId.toString(), ProposalStatus.CANCELED);
});

indexer.onEvent({ contract: "ZKGovernor", event: "ProposalExecuted" }, async ({ event, context }) => {
  await updateProposalStatus(context, event.params.proposalId.toString(), ProposalStatus.EXECUTED);
});

indexer.onEvent({ contract: "ZKGovernor", event: "ProposalQueued" }, async ({ event, context }) => {
