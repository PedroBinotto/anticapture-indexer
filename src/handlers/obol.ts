...
indexer.onEvent({ contract: "ObolToken", event: "Transfer" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "ObolToken", event: "DelegateChanged" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "ObolToken", event: "DelegateVotesChanged" }, async ({ event, context }) => {
...
// Governor handlers
indexer.onEvent({ contract: "ObolGovernor", event: "VoteCast" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "ObolGovernor", event: "ProposalCreated" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "ObolGovernor", event: "ProposalCanceled" }, async ({ event, context }) => {
  await updateProposalStatus(context, event.params.proposalId.toString(), ProposalStatus.CANCELED);
});

indexer.onEvent({ contract: "ObolGovernor", event: "ProposalExecuted" }, async ({ event, context }) => {
  await updateProposalStatus(context, event.params.proposalId.toString(), ProposalStatus.EXECUTED);
});

indexer.onEvent({ contract: "ObolGovernor", event: "ProposalQueued" }, async ({ event, context }) => {
