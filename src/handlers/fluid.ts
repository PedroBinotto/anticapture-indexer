...
indexer.onEvent({ contract: "FLUIDToken", event: "Transfer" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "FLUIDToken", event: "DelegateChanged" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "FLUIDToken", event: "DelegateVotesChanged" }, async ({ event, context }) => {
...
// Governor handlers
indexer.onEvent({ contract: "FLUIDGovernor", event: "VoteCast" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "FLUIDGovernor", event: "ProposalCreated" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "FLUIDGovernor", event: "ProposalExtended" }, async ({ event, context }) => {
...
indexer.onEvent({ contract: "FLUIDGovernor", event: "ProposalCanceled" }, async ({ event, context }) => {
  await updateProposalStatus(context, event.params.proposalId.toString(), ProposalStatus.CANCELED);
});

indexer.onEvent({ contract: "FLUIDGovernor", event: "ProposalExecuted" }, async ({ event, context }) => {
  await updateProposalStatus(context, event.params.proposalId.toString(), ProposalStatus.EXECUTED);
});

indexer.onEvent({ contract: "FLUIDGovernor", event: "ProposalQueued" }, async ({ event, context }) => {
